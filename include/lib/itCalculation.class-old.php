...
class ItCalculation {
	...
	/*	$pt_val ="SELECT IF(display_flag=1,1,0) is_pt FROM company_pay_structure WHERE pay_structure_id='c_pt'";
		
		if ($result = mysqli_query ( $this->_connection, $pt_val )) {
			$row = mysqli_fetch_array ( $result, MYSQLI_ASSOC );
				//print_r($row);
				//die();
		}*/
	...
	private function yearlyTax($row) {
		...
		$tax = $tax - $this->rebate;
		 if($this->taxable_income > $this->surcharge_percentage1_applicable_to && $this->taxable_income < $this->surcharge_percentage1_applicable_to)
			$sur_charge = $tax * ($this->surcharge_percentage1/100);
		elseif($this->yearly_gross > $this->surcharge_percentage2_applicable_to)
			$sur_charge = $tax * ($this->surcharge_percentage2/100);
		else 
			$sur_charge = 0;
		$tax += $sur_charge;
		$edu_chess = $tax * 0.03;
