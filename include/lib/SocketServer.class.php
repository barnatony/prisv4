...
class SocketServer
{
	...
	/*! @function __construct
	 ...
	 @result void
	 */
	public function __construct($bind_ip,$port,$close=null)
	{
		...
		if(!socket_bind($this->master_socket,$this->config["ip"],$this->config["port"])){
		 SocketServer::debug("Issue Binding on {$bind_ip}:{$port} Error:".socket_strerror(socket_last_error($this->master_socket)));
		 if($close){
		 	//if close is set shutdown the socket
		 	socket_shutdown ($this->master_socket);
		 	SocketServer::debug(" Socket Shutdown {$bind_ip}:{$port} Error: ".socket_strerror(socket_last_error($this->master_socket)));
		 	//close the socket
		 	socket_close ( $this->master_socket );
	 		SocketServer::debug(" Closing socket {$bind_ip}:{$port}");
		 }
		 die();
		}
	...
	{
		$log_file = fopen("listener_error_log.txt", 'a');
		fwrite($log_file,"-- Socket Err: ".date('Y-m-d h:i:s').$text. "\r\n");
		fclose($log_file);
	}
