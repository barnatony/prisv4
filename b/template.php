...
class Template {
    ...
    private function build_nav_array($nav_array) {
        foreach ($nav_array as $node) {
            ...
            if ($link_type == 'heading') {
                ...
                if ($link_sub) {
                	$link_active = $sub_active = $this->build_nav_array_search($node['sub']) ? true : false;
                }
                ...
                // Add the link
                $link_url = (strpos($link_url, 'http://') !== false) ?$link_url:myUrl($link_url); 
    ...
    private function build_nav_array_search($nav_array) {
        foreach ($nav_array as $node) {
        	//echo $node['url']." - ".$this->main_nav_active."<br>";
        if (isset($node['url']) && (basename($node['url']) == $this->main_nav_active)) {
        	return true;
    ...
    function load_default_properties(){
    	$this->author                       = 'PRIS HRLabz';
    	$this->robots                       = 'noindex, nofollow';
    	$this->title                        = 'PRIS HRLabz';
    	$this->description                  = 'PRIS';
    	$this->keywords                     = 'PRIS, a cloud based HRMS application available at very competitive pricing, for more details call +919841616416';
    	$this->meta_images                  = '';
    	$this->meta_Url                     = '';
    	
    	// Global Included Files (eg useful for adding different sidebars or headers per page)
    	$this->inc_sidebar               = VIEW_PATH.'menu/sidebar.php';
    	$this->inc_header                = VIEW_PATH.'menu/frontend_header_navigation.php';
    	$this->inc_footer                = VIEW_PATH.'menu/footer.php';
    	$this->l_sidebar_position        = true;
    	$this->l_sidebar_visible_desktop = true;
    	$this->l_side_scroll             = true;
		$this->l_sidebar_mini            = true;
    	
    	
    	// Global Color Theme
    	$this->theme                        = 'prisui';       // '' for default theme or 'amethyst', 'city', 'flat', 'modern', 'smooth'
    	
    	// Global Cookies
    	$this->cookies                      = false;    // True: Remembers active color theme between pages (when set through color theme list), False: Disables cookies
    	
    	// Global Body Background Image
    	$this->body_bg                      = '';       // eg 'assets/img/photos/photo10@2x.jpg' Useful for login/lockscreen pages
    	
    	// Global Header Options
    	$this->l_header_fixed               = true;     // True: Fixed Header, False: Static Header
    	
    	// Global Sidebar Options
    	$this->l_sidebar_visible_mobile     = false;    // True: Visible Sidebar (< 992px), False: Hidden Sidebar (< 992px)
    	
    	// Global Side Overlay Options
    	$this->l_side_overlay_hoverable     = false;    // True: Side Overlay hover mode (> 991px), False: Disable hover mode
    	$this->l_side_overlay_visible       = false;    // True: Visible Side Overlay, False: Hidden Side Overlay
