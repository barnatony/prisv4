$.validator.addMethod("regx", function(value, element, regexpr) {          
  ...
  $("#js-validation-reminder").validate({
    ...
    submitHandler:function(element){
       usermail = $("#reminder-email").val();
        $.ajax({
          type:"POST",
          url: App.myUrl('users/password_reset_request/'+usermail),
          ...
          success: function(data){
        	  if(isJson(data)==false)
        		  $(".err-notification").empty().html('<div class="alert alert-danger alert-dismissable">\
                          <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>\
                          <p class="push-15">No User found. &nbsp;&nbsp;&nbsp;&nbsp;Error <a class="alert-link" href="javascript:void(0)">occured</a>!</p>\
                      </div>').show().fadeOut(3000);
        	  else
        		  data = JSON.parse(data);
        	if(data[0]==true){
        		$(".err-notification").html('Password reset link is sent to your registered email.').addClass("text-success");
        		$('#js-validation-reminder')[0].reset();
        		$('#js-validation-reminder').hide();
        	}
