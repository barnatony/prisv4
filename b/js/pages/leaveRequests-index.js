...
var datatables = function() {
	//privilages(3) hr,employee,team_lead
	...
	var initAddressDataTable = function() {
		$myteam_dt = jQuery('.myteam-dataTable').DataTable({
            ...
            "autoWidth": false,
            "searching": true,
            "dom": 'lfrtip<"clear"><"push-10 text-right" B>',//"fitBp",
            
            "sAjaxSource":App.myUrl("leaveRequests/dt_leave_requests/me/"),
            "fnServerData": function ( sSource, aoData, fnCallback ) {
            	...
            	"dataType": 'json',
            	"type": "GET",
            	"url": App.myUrl("leaveRequests/dt_leave_requests/me/"),
            	"data": aoData,
            	"success": fnCallback
            	} );
            ...
            "columnDefs": [
                ...
                },
                {
                    "render": function ( data, type, row ) {
                    	if(row["privilage"]=="employee"){ //for employee
                    		...
                    		var today=Date.parse(moment().format("YYYY-MM-DD"));
                    		
                    		
	                    		if(row["status"]=='A' && compare_date>=today)
	                        		return '<button class="btn btn-rounded btn-sm btn-danger-outline btn-danger push-10-l cancel" data-value="C" data-id="'+row["id"]+'" type="button" title="Cancel leave"><i class="fa fa-times"></i></button>';
	                        	else if(row["status"]=='A')
	                        		return '<div class="text-center push-50-r">-</div>';
	                        	else if(row["status"]=='RQ')
	                        		return '<button class="btn btn-rounded btn-sm btn-danger-outline btn-danger push-10-l withdraw" data-value="W" data-id="'+row["id"]+'" type="button" title="Withdraw leave"><i class="fa fa-times"></i></button>';
                ...
                /*initComplete:function ( settings, json) {
                	var columnsCustom =[8,9];
                	$myteam_dt.columns(columnsCustom).every( function () {
        ...
        var detailRows = [];
        //$("#myteam-datatable_filter").on("keyup", 'input', function() {
        	//console.log('dasd');
        	//var Svalue=this.value;
        	
           //console.log($myteam_dt.search( this.value ).draw());
        //});
    ...
    function date(d) { 
    	d.minDate = $('#startDate').val(); 
    	d.maxDate = $('#endDate').val();
...
$(document).on('click', '.withdraw,.cancel', function (e) {
    e.preventDefault();
    var id = $(this).data("id");
