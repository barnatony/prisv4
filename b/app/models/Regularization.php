...
function update(){
	$reason=array("Missed In Punch","Missed Out Punch");
	if(in_array($this->get('reason_type'),$reason)&& $this->get('status')=='A'){
...
function InsertIntoBiometric($empId,$day,$type,$reason){
	...
	$cond = "";
	$temp_date_time = explode("@", $reason);
	$date_time = trim($temp_date_time[1]);
	if($date_time != ""){
		$cond = $day." ".$date_time;
		$cond="CONCAT(:day,' ','".$date_time."')";
	}else{
