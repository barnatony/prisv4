<?php
function _view($for=null,$month){
	$dateStatus=isset($_REQUEST['dateStatus'])?$_REQUEST['dateStatus']:"";
	...
	//for differentiate dashboard or attendance
	if(!$month){
		if($companyProp['attendance_dt'] !=1){
			...
			//$startDate = date('Y-m-t',$start_date);
			//$endDate = date('Y-m-t',$end_date);
		}
	}else{
		if($dateStatus=="daily"){
		...
		if($companyProp['attendance_dt'] !=1){
			...
			//$endDate = date('Y-m-t',strtotime($startDate));
		$startDate = date('Y-m-t',strtotime($_REQUEST['start_date']));
	$endDate =date('Y-m-t',strtotime($_REQUEST['end_date']));
			}
		}
	}
...
");*/
 ...
 $query ="SELECT ID,NAME,shift_name,work_day,team_name,DATE,SUBSTRING_INDEX(check_in,':',2) check_in,SUBSTRING_INDEX(check_out,':',2) check_out,all_punches,
      ...
      ,IFNULL(SUBSTRING_INDEX(IF(late!='-' AND earlyout!='-',IF((day_type='SH' OR is_weekday='SH'),late,IF(ADDTIME((late),IF(earlyout>shift_hrs,shift_hrs,earlyout))>shift_hrs,shift_hrs,ADDTIME((late),IF(earlyout>shift_hrs,shift_hrs,earlyout)))),IF(late!='-',IF((day_type IN ('FD','FH') OR is_weekday IN('FD','FH')),'-',late),IF((day_type IN ('FD','SH') OR is_weekday IN('FD','SH')),'-',IF(earlyout>shift_hrs,shift_hrs,earlyout)))),':',2),'-') total_time,
	  DATE_FORMAT(TIMEDIFF(out_punch,in_punch),'%H:%i') worked_hrs
FROM (
SELECT employee_id ID,employee_name NAME,shift_name,work_day,DATE_FORMAT(work_day,'%a %d %b,%Y') DATE,DATE_FORMAT(check_in,'%T') check_in,IF(DATE_FORMAT(check_in,'%Y-%m-%d') != DATE_FORMAT(NOW(),'%Y-%m-%d'),DATE_FORMAT(check_out,'%T'),'-') check_out,all_punches,
    check_in In_time,check_out OUT_time,is_day,late_end,
    ...
    (CASE WHEN DATE_FORMAT(check_in,'%Y-%m-%d') = DATE_FORMAT(NOW(),'%Y-%m-%d') OR (is_day=1 AND check_out >CONCAT(DATE_FORMAT(work_day,'%Y-%m-%d'),' ',shift_end_time,':00')) 
          ...
          ELSE '-' END) earlyout,
    IF((day_type='SH' OR is_weekday IN('FD','SH')) AND early_stat IS NOT NULL,'-',IFNULL(early_stat,'-')) early_status,IF((day_type='FH' OR is_weekday IN('FD','FH')) AND late_stat IS NOT NULL,'-',IFNULL(late_stat,'-')) late_status,shift_hrs,team_name,day_type,is_weekday,check_in in_punch,check_out out_punch
FROM (
    SELECT employee_id,ref_id,shift_id,from_date,to_date,DATE_FORMAT(work_day,'%Y-%m-%d') work_day,
    (CASE WHEN is_day=1 THEN MIN(date_time)
...
GROUP BY employee_id,work_day,regularize_type
	...
	//to get worked hrs for daily table
	$q_daily = $dbh->prepare("SELECT a.employee_id,w.employee_name,a.shift_id,a.shift_name,a.shift_st_time start_time,a.shift_end_time end_time,a.days dates,
						       a.shift_hrs,IFNULL(DATE_FORMAT(a.checkIn,'%H:%i'),'') check_in,IFNULL(DATE_FORMAT(a.checkOut,'%H:%i'),'') check_out,SUBSTRING_INDEX(a.tot_worked,':',2) tot_worked,work_hrs worked_hours,-- a.day_type, ab.day_type,
						       IF(ab.day_type='FH','-',SUBSTRING_INDEX(a.lateIn,':',2)) late,IF(ab.day_type='SH','-',SUBSTRING_INDEX(a.earlyOut,':',2)) early_out,a.ot,a.punches all_punches
							FROM attendance_summary a
							INNER JOIN employee_work_details w
							ON a.employee_id = w.employee_id
							LEFT JOIN emp_absences ab
							ON a.employee_id = ab.employee_id AND a.days = ab.absent_date
							WHERE a.employee_id=:empId AND a.days BETWEEN :startDate AND :endDate;");
	
	$q_daily->bindParam('empId',$empId);
	$q_daily->bindParam('startDate', $startDate);
	$q_daily->bindParam('endDate', $endDate);
	$q_daily->execute();
	//$q_daily=$q_daily->fetch(PDO::FETCH_ASSOC);
	
	$Q_daily = array();
	while($row = $q_daily->fetch(PDO::FETCH_ASSOC)){
		$Q_daily[] =$row;
	}
...
;">'.date('d', strtotime($key)).'</br>'.date('D', strtotime($key)).'</th>';
    ...
    $attendance=array("attendances"=>$attendances,"Allpunches"=>array(),"status"=>$status,"daily"=>$Q_daily);
