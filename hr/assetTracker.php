									...
									<div class="form-group">
										...
										<div class="col-lg-7">
											<input class="form-control" type="text" name="assetId"
												maxlength="8" /> <span class="nm_not_found"></span>
										</div>
									</div>
									<div class="form-group">
	...
	<script type="text/javascript">
           $(document).ready(function () {
        	  // $('#add-assetTracker-toggle').toggle('hide');
               ...
               $('#purDate,#wed,#e_purDate,#e_wed').datetimepicker({
            	  format: 'DD/MM/YYYY',
             });
                   ...
                   return {
                       ...
                       init: function () {
                           ...
                           $(document).on('click', "#assetTracker-sample a.enable", function (e) {
                            ...
                            $('#mfr').val(data[9]);
                            $('#model').val(data[10]);
                            $("#e_status option[value='" + data[11] + "']").prop("selected", true);
                           ...
                           $('#assetTrackerForm').validate({
								rules:{
                           		...
                           		messages:{
                           			assetId: "<label class='text-danger'>Asset Id is Required</label>",
									atName: "<label class='text-danger'>Asset Name is Required</label>",
									purDate:"<label class='text-danger'>Purchase Date is Required</label>",
									cost:"<label class='text-danger'>Cost is Required</label>",
									wed:"<label class='text-danger'>Warranty Expiry Date is Required</label>",
									sNo:"<label class='text-danger'>Serial No is Required</label>",
									mfr:"<label class='text-danger'>Manufacture is Required</label>",
									model:"<label class='text-danger'>Model is Required</label>"
           ...
           $('#assetId').on('blur', function (e) {
   			if($(this).val()!= ""){
   				$.ajax({
   					url: "php/assetTracker.handle.php",
   					data:{act: '<?php echo base64_encode($_SESSION['company_id']."!checkAssetId");?>',assetId:$(this).val()},			
   					type: "POST",
   		    		success:function(data){
   			    		data=JSON.parse(data);
   			    		if(data[0] == 'error'){
   			    		 $('#assetId').removeClass('correct');
                         $('#assetId').addClass('wrong');
                         $('.nm_not_found').fadeIn("Fast");
                         $('.nm_not_found').html('Asset Id Already Found Try Other');
                         $('.nm_not_found').fadeOut(3000);
   			    		}else if(data[0] == 'success'){
   			    			$('.nm_not_found').hide();
   			    			$('#assetId').removeClass('wrong');
   			    			$('#assetId').addClass('correct');
