...
switch ($action) {
	...
	case "data_feed" :
		...
		break;
	case "epfReport" :
		require_once (dirname ( dirname ( dirname ( __FILE__ ) ) ) . "/include/lib/epfReportV2.class.php");
	...
	case "esiReport" :
		...
		$result = $esiReport->generateEsiReportData ( $esiReport->monthYear );
	...
	case "epfReportTxt" :
		...
		$headArray = array ("UAN","employee_name","GROSS","EPF","EPS","EDLI","EE_Remitted","EPS_Remitted","ER_Remitted","ncp_days","refund_advances");
		foreach ( $epfReport->tableData as $rowData ) {
			for($i = 0; $i < count ( $epfReport->tableHeaders ); $i ++) {
				if($i==0 || $i==10){
					if($i==0){
						$UAN = explode('_',$rowData[$headArray[0]]);
						fwrite ( $handle, "{$UAN[0]}#~#" );
					}else if($i==10){
						fwrite ( $handle, "{$rowData[$headArray[$i]]}" );
					}
				}else
					fwrite ( $handle, "{$rowData[$headArray[$i]]}#~#" );
			}
	...
	case "tdsReport" :
		...
		$tdsReport->generateTdsData ( 'c_it', $tdsReport->monthYear );
