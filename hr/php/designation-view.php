...
while ( $aRow = mysqli_fetch_array ( $main_query ) ) {
	...
	for($i = 0; $i < count ( $columns ); $i ++) {
		if ($columns [$i] == 'enabled') {
			...
			$row [] = ($aRow [$columns [$i]] == "1") ? '<a class="designation_disable"  title="Disable" href="">
<button class="btn btn-success btn-xs" style="padding: 1px 4px;"><i class="fa fa-unlock"></i></button>      
</a><a href="#designation_edit" title="Edit"  class="designation_edit">
<button class="btn btn-primary btn-xs" ><i class="fa fa-pencil"></i></button></a>' : '<a class="designation_enable" title="Enable"  href="">
 <button class="btn btn-danger  btn-xs" style="padding: 1px 6px;"><i class="fa fa-lock"></i></button></a>
<a href="#designation_edit" title="Edit"  class="designation_edit">
