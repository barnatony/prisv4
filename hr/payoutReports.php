...
include_once (__DIR__ . "/header.php");
      ...
      <!--header end-->
		...
		<section id="main-content">
			<section class="wrapper site-min-height">
				<!-- page start-->
				<section class="panel">
					<header class="panel-heading"> Bank Payout Statement </header>
					<div class="panel-body">
						<form class="form-horizontal" id="bankPayoutReportForm">
							<div class="col-lg-12 row">
								...
								<div class="col-sm-2">
									<input class="btn btn-sm btn-success" id="bankPayoutReport"
								...
								<form class="form-horizontal" target="foo();"
									...
									<div id="visibleIfnodata" class="hide">
										<h6 class="container" style="width: 50%; text-align: center;">
											<strong>No Available Data For Bank Payout Reports</strong>
										</h6>
									</div>
									...
									<input type="hidden" id="ptdate" name="dateOfreports"> <input
										type="hidden" value="bankPayout" name="report">
								</form>
	...
	<script>
     ...
     $(document).on('click', "#bankPayoutReport", function (e) {
        e.preventDefault();
     ...
     $(document).on('click', "#export-to-excel", function (e) {
      ...
      //export table
       $("#example1").table2excel({
			...
			filename: 'Bank_Payout_Reports_OF'+$('#monthDate').val(),
			exclude_img: true,
			exclude_links: true,
			exclude_inputs: true
		});         
     ...
     function ptTableFormat(date){
      $.ajax({
         ...
         cache: false,
         data: {act: '<?php echo base64_encode($_SESSION['company_id']."!bankPayoutReport");?>',date:date},
         success: function (data) {
        	 ...
        	 var n = ["","January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
       	    $('.HeadingTable').html("Bank Payout Reports For The Month Of "+n[Number(dateNo[0])]+" "+dateNo[1]);
     	  ...
     	  for (var i = 0, len = jsonobject.tableData.length; i < len; ++i) {
     		 if(i % 2 == 0){
     		   ...
     		   $.each(jsonobject.tableData[i], function (k, v) {
         		   if(k=='countEmp' || k=='net' ){
             		   if(i % 2 == 0){
         				 html += '<td  style="text-align:right;background: #E0F9FF;color: #184A47;">' +  reFormateByNumber(v) + '</td>';
               		    }else{
               		     html += '<td  style="text-align:right;color:#2A827C">' +  reFormateByNumber(v) + '</td>';
               		    }
