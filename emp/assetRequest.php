				...
				<section class="panel">
					...
					<form class="form-horizontal" role="form" method="post"
						...
						<input type="hidden" name="act"
							value="<?php echo base64_encode($_SESSION['company_id']."!insertByEmp");?>" />
						<div class="col-lg-12">
								...
								<div class="col-lg-6">
									...
									<div class="form-group">
										...
										<div class="col-lg-7">
											...
											</select> <label class="type_asset">Please select The Type</label>
										</div>
	...
	<script type="text/javascript">
           $(document).ready(function () {
        	  // $('#add-assetTracker-toggle').toggle('hide');
               ...
               $('#fromDate,#wed,#toDate,#e_wed,#issued_on').datetimepicker({
            	   format: 'DD/MM/YYYY',          	 
               }); 
           ...
           function viewAsset(value){
        	   $('.ajax_loader').show();
                            ...
                            //Edit claims
           $(document).on('click','#panel_heading',function (e) {
               	                e.preventDefault();
               	             $('#show_panel_d').hide();
               	          	 $('#table_show').show();
          ...
          $('#atSubmit').click(function(e){
            		  ...
            		  $('.type_asset').hide();
                           ...
                           $('#assetRequestForm').validate({
                        	   rules: {
                				...
                				messages: {
                					fromDate: "Please Enter your From Date",
                					toDate: "Please Enter your To Date",
                					purpose:"Please Enter your Purpose",
                					desc:"Please Enter From Description",
           ...
           function getAssetRequest()
           {
        	   ...
        	   $.ajax({
                   ...
                   url: "php/assetRequest.handle.php",
                   cache: false,
                   data: {act: '<?php echo base64_encode($_SESSION['company_id']."!selectAssetRequestByEmp");?>'},	
                   success: function (data) {
                       data = JSON.parse(data);
                      $('#getAssetRequest').html("");
                       if (data[0] == "success") {
                           ...
                           for(lc=0;lc<data[2].length;lc++){
                            ...
                            var status = '<a class="btn  view btn-sm btn-success">Issued</a>';
                            var download = '<form action="php/assetRequest.handle.php" method="post" id="generate_pdf_form"><input type="hidden" name="employee_id" id="employee_id" value='+data[2][lc].employee_id+'><input name="request_Id" class="requestId_generate" value='+data[2][lc].request_id+' type="hidden"><input type="hidden" name="act" id="act" value="<?php echo base64_encode($_SESSION["company_id"]."!downloadGeneratePdf");?>"><button type="button" class="btn btn-xs btn-primary" data-toggle="tooltip" title="Download Issue Bill" style="margin-left:10px;"><i class="fa fa-file-pdf-o generate_pdf" id="generate_pdf"></i></button></form>';
                            }else if(data[2][lc].status == 'R'){
                            var status = '<a class="btn  view btn-sm btn-primary">Returned</a>';
                            }else if(data[2][lc].status == 'D'){
                            var status = '<a class="btn  view btn-sm btn-danger">Declined</a>';
                            }   
                          	html +='<tr><td>'+data[2][lc].employee_name+'</td><td>'+data[2][lc].asset_type+'</td><td>'+data[2][lc].date+'</td><td>'+data[2][lc].purpose+'</td><td>'+status+'</td><td class="text-center"><a style="cursor:pointer;float:left;"title="View" class="asset_view"  data-value='+data[2][lc].request_id+'><button class="btn btn-primary btn-xs"><i class="fa fa-eye"></i></button></a>'+download+'</td></tr>'
                         }
                       	...
                       	$('#getAssetRequest').html('<tr><td colspan="7" class="claims_not" id="false"><input type="hidden" name="asd" id="asd" value="NO"/>No Asset Request Found</td></tr>');
